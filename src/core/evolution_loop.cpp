                ...
                for (int v = 0; v < nv; ++v) {
                    ...
                    for (int k = 0; k < nz; ++k)
                        for (int j = 0; j < ny; ++j) {
                            ...
                            Real r_int_lo = std::sqrt(x_int_lo * x_int_lo + y * y + z * z) + 1e-30;
                            Real x_int_hi = g.x(0, iex);
                            Real r_int_hi = std::sqrt(x_int_hi * x_int_hi + y * y + z * z) + 1e-30;
                    ...
                    for (int k = 0; k < nz; ++k)
                        for (int i = 0; i < nx; ++i) {
                            ...
                            Real r_int_lo = std::sqrt(x * x + y_int_lo * y_int_lo + z * z) + 1e-30;
                            Real y_int_hi = g.x(1, iey);
                            Real r_int_hi = std::sqrt(x * x + y_int_hi * y_int_hi + z * z) + 1e-30;
                    ...
                    for (int j = 0; j < ny; ++j)
                        for (int i = 0; i < nx; ++i) {
                            ...
                            Real r_int_lo = std::sqrt(x * x + y * y + z_int_lo * z_int_lo) + 1e-30;
                            Real z_int_hi = g.x(2, iez);
                            Real r_int_hi = std::sqrt(x * x + y * y + z_int_hi * z_int_hi) + 1e-30;
            ...
            for (int kk = 0; kk < tnz; ++kk)
                for (int jj = 0; jj < tny; ++jj)
                    for (int ii = 0; ii < tnx; ++ii) {
                        ...
                        Real det = gxx * (gyy * gzz - gyz * gyz) - gxy * (gxy * gzz - gyz * gxz) +
                            gxz * (gxy * gyz - gyy * gxz);
