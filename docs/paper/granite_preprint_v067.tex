...
\url{https://github.com/LiranOG/Granite-NR}, archived at Zenodo with
...
\paragraph{Hardware availability and benchmark scope.}
The benchmark suite presented in this section reflects the computational
resources available at the time of writing.
A subset of the planned validation runs---specifically, the three-resolution
gauge-wave convergence study required to demonstrate formal fourth-order
convergence of the CCZ4 advection stencil, the OpenMP thread-scaling
measurement across the full $\{1,2,3,6\}$-thread matrix, and the matched
binary black-hole convergence series---have not yet been executed.
Access to Machine~B (Intel Core i5-12600KF, 32\,GB DDR4-3600), which was
previously used to supplement the desktop node for computationally intensive
runs, is no longer available; the remaining development hardware (Machine~A,
Sec.~\ref{sec:benchmarks}) is insufficient to execute high-resolution
multi-hour campaigns without disrupting active code-development iterations.
These outstanding runs are designated as v0.7 deliverables: once stable
hardware access is restored---either through a replacement workstation or
an allocated HPC partition---all three benchmark families will be completed
and the formal convergence orders reported.
Until that point, correctness of the CCZ4 and GRMHD modules is supported
by the 92-test GoogleTest suite (Sec.~\ref{sec:architecture}, Table~\ref{tab:tests}) and the
single-puncture and binary evolution results shown below, which demonstrate
constraint stability and monotonic decay across $500\,M$ of evolution.
...
The central lapse diagnostic reads $\alpha_{\rm center} \approx 0.94$
at AMR level~0; the expected trumpet value at the finest level near
the puncture is $\alpha \approx 0.3$~\cite{Campanelli2006}.
See the known diagnostic limitation in Sec.~\ref{sec:limitations}.
...
\paragraph{Note on hardware availability.}
The thread-scaling measurements reported in this section were performed on
Machine~B prior to the loss of access to that hardware.
The planned extension of the scaling study to a $128^3$ base grid---which
would validate the predicted $\eta_\parallel \gtrsim 60\%$ regime---is
deferred to v0.7, together with the full gauge-wave and BBH convergence
campaigns.
...
\begin{tabular}{p{6cm}p{4.5cm}cc}
  ...
  No GRMHD or radiation benchmarks validated to date
    ...
    & Known & v0.7 \\[2pt]
    Three-resolution gauge-wave convergence study, thread-scaling extension
    to $128^3$, and matched BBH convergence series not yet executed
    & Formal 4th-order convergence order undemonstrated;
      SXS:BBH:3634 validation pending
    & Known & v0.7 \\[2pt]
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Binary black hole inspiral}: $61.3\times$ constraint reduction
    ...
    Bowen--York initial-data construction and stable long-term evolution.
  \item \textbf{Thread safety}: bit-for-bit identical physics output across
    ...
    Full thread-scaling to $128^3$ base resolution is a v0.7 deliverable,
    pending restoration of the second development machine.
...
The three-resolution gauge-wave convergence study, the matched BBH
convergence series, and the $128^3$ thread-scaling extension are
additionally pending restoration of dedicated hardware for long-running
campaigns; these are v0.7 deliverables and will accompany a revised
arXiv submission.
