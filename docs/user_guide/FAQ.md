...
**Q: What physical scenarios can GRANITE simulate in its current state (v0.6.8)?**
...
A: Dynamic regridding is **fully implemented and active in v0.6.8**. Gradient-based and puncture-tracking triggers fire per step via `AMRHierarchy::subcycle()`, with iterative union-merge box aggregation and live regridding integrated into the main RK3 loop. If your block count is not changing, verify that your refinement thresholds in `params.yaml` are appropriate for your resolution.
