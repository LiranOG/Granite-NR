...
def main() -> None:
    parser = argparse.ArgumentParser(description="GRANITE Developer Benchmark Dashboard")
    parser.add_argument(
        "logfile", nargs="?", type=Path, help="Path to telemetry log file. If omitted, reads from stdin."
    )
    parser.add_argument("--benchmark", type=str, default="single_puncture", help="Benchmark run name")
